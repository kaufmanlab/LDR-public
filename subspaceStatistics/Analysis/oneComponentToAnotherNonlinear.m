function output = oneComponentToAnotherNonlinear
%% OVERVIEW
...
% Refers to the kernel type and length scale.
kernelType = 'OU';
lengthScale = 500;
...
for monkey = 1:size(ShenoyMonkeyData,2)
    ...
    output(monkey).M1 = analyze(ShenoyMonkeyData(monkey).M1,L2,svEnergy, ...
        leaveOut,basisFxnNum(monkey).M1.maxDim,kernelType,lengthScale);
    output(monkey).PMd = analyze(ShenoyMonkeyData(monkey).PMd,L2,svEnergy, ...
        leaveOut,basisFxnNum(monkey).PMd.maxDim,kernelType,lengthScale);
end
...
function out = analyze(data,L2,svEnergy,leaveOut,factorDim,kernelType,lengthScale)
data = pruneRepeats(data);
% Perform prediction.
[out.data,out.prediction,reassambleTensor] = predictOneComponentFromAnotherNonlinear( ...
...
[out.nullData,out.nullPrediction] = predictOneComponentFromAnotherNonlinear( ...
    out.nullData,leaveOut,L2,factorDim,svEnergy,kernelType,lengthScale);
% Quantify the variance explained.
% for predictorIndex = 1:ceil(factorDim/2)
%     for predictedIndex = 1:ceil(factorDim/2)
%         for cond = 1:size(data,2)
